...
create table Trades (
    ...
    CONSTRAINT `buysell` CHECK (buysell IN ('B', 'S')), -- parsed but not implement in MySQL 5.7
    CONSTRAINT `state` CHECK (state IN ('P', 'M', 'C', 'A', 'D', 'E', 'S', 'R')) -- parsed but not implement in MySQL 5.7
);
